...
def build_scopus_query(components: Dict[str, Any]) -> str:
    ...
    if submitted:
        with st.status("Searching...", expanded=True) as status:
            try:
                if database == "PubMed":
                    # PubMed search
                    pmids = pubmed_search(search_query or "", max_results)
                    if not pmids:
                        st.warning("No results found")
                        return

                    records = pubmed_fetch_details(pmids)
                    results = pubmed_repo.store_results(review_id, search_query or "", records)
